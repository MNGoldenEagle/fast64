...
from ..oot_constants import ootData, ootEnumSceneSetupPreset, ootEnumCamTransition, ootEnumTransitionAnims
...
class OOTEntranceProperty(PropertyGroup):
    ...
    # Cutscene ID Options
    showTitlecard: BoolProperty(name="Show Title Card?")
    startCutscene: BoolProperty(name="Start Cutscene?")
    cutsceneId: IntProperty(min=0, max=127)
        ...
        if not entranceProp.showTitlecard:
            ...
            if entranceProp.startCutscene:
                ...
                if entranceProp.cutsceneId > len(sceneObj.extraCutscenes):
                    box.label(text="The cutscene ID exceeds the number of cutscenes that exist in the scene.")
