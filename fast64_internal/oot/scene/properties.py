...
class OOTExitProperty(PropertyGroup):
    ...
    def draw_props(self, layout: UILayout, index: int, headerIndex: int, objName: str):
        box = layout.box()
        box.prop(self, "expandTab", text=f"Exit {str(index + 1)}", icon="TRIA_DOWN" if self.expandTab else "TRIA_RIGHT")
        if self.expandTab:
            drawCollectionOps(box, index, "Exit", headerIndex, objName)
            exitGroup = box.column()
            exitGroup.prop(self, "scene", text="Scene")
            exitGroup.prop(self, "entranceId", text="Entrance ID")
...
class OOTSceneTableEntryProperty(PropertyGroup):
    ...
    titleCardType: EnumProperty(name="Format", items=enumTexFormat, default="IA8")

    def draw_props(self, layout: UILayout):
        drawEnumWithCustom(layout, self, "drawConfig", "Draw Config", "")
        layout.prop(self, "hasTitle")
        if self.hasTitle:
            layout.prop(self, "titleCard")
            if not os.path.exists(self.titleCard) or \
                not self.titleCard.endswith(".png") or \
                len(getImageSize(self.titleCard)) != 2:
