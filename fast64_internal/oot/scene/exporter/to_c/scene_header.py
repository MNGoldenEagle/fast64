...
def getExitList(outScene: OOTScene, headerIndex: int):
    exitList = CData()
    declarationBase = f"ExitEntry {outScene.exitListName(headerIndex)}[{len(outScene.exitList)}]"
    ...
    exitList.source = (
        ...
        + "\n".join(f'{indent}{{ {exit.sceneIndex}, {exit.entranceIndex}, {exit.fadeOutAnim}, {{ {"true" if exit.continueBGM else "false"} }} }},' for exit in outScene.exitList)
        + "\n};\n\n"
    )
