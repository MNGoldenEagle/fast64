...
def getSpawnListCmd(outScene: OOTScene, headerIndex: int):
    return f"{indent}SCENE_CMD_ENTRANCE_LIST({len(outScene.entranceList)}, {outScene.entranceListName(headerIndex) if len(outScene.entranceList) > 0 else 'NULL'})"
...
def getExitListCmd(outScene: OOTScene, headerIndex: int):
    return f"{indent}SCENE_CMD_EXIT_LIST({len(outScene.exitList)}, {outScene.exitListName(headerIndex)})"
