...
def ootCameraDataToC(camData):
    ...
    if len(camData.camPosDict) > 0:
        camDataName = f"BgCamInfo {camData.camDataName()}[{str(len(camData.camPosDict))}]"

        camC.source = f"{camDataName} = {{\n"
        ...
        if camPosIndex > 0:
            posDataName = f"Vec3s {camData.camPositionsName()}[{str(camPosIndex)}]"
            posC.header = f"extern {posDataName};\n"
            posC.source = f"{posDataName} = {{\n{posC.source}"
        else:
            posC = CData()
    else:
...
def ootCollisionToC(collision):
    ...
    if len(collision.polygonGroups) > 0:
        data.header += f"extern SurfaceType {collision.polygonTypesName()}[];\n"
        data.header += f"extern CollisionPoly {collision.polygonsName()}[];\n"
        polygonTypeC = f"SurfaceType {collision.polygonTypesName()}[] = {{\n"
        polygonC = f"CollisionPoly {collision.polygonsName()}[] = {{\n"
        polygonIndex = 0
    ...
    if len(collision.vertices) > 0:
        data.header += f"extern Vec3s {collision.verticesName()}[{str(len(collision.vertices))}];\n"
        data.source += f"Vec3s {collision.verticesName()}[{str(len(collision.vertices))}] = {{\n"
    ...
    if len(collision.waterBoxes) > 0:
        data.header += f"extern WaterBox {collision.waterBoxesName()}[];\n"
        data.source += f"WaterBox {collision.waterBoxesName()}[] = {{\n"
    ...
    data.header += f"extern CollisionHeader {collision.headerName()};\n"
    data.source += f"CollisionHeader {collision.headerName()} = {{\n"

    if len(collision.bounds) == 2:
        for bound in range(2):  # min, max bound
            for field in range(3):  # x, y, z
                data.source += f"\t{str(collision.bounds[bound][field])},\n"
    else:
